...
- **The work search has no Linear issue. Do not open one.** The Pinole work API is the ledger of record (postings and activities, read and written through the `pinole` CLI), and the cadence lives on the calendar as the recurring 🔎 Weekly Work Search block. When a session finishes the week's slate, the applications and supporting activities get logged as activities (`pinole work activities log`) and nothing else is minted.
