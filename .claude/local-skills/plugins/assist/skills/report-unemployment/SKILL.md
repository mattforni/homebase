...
- **The activities ledger**: the Pinole work API through the `pinole` CLI. The slate is `pinole work activities list --week <YYYY-Www> --table` for the claim week; the confirmation goes back with `pinole work activities report --confirmation <code> <id> [<id>...]` on every included activity id, and an activity left off the slate gets `pinole work activities exclude <id> --reason <text>` so the ledger records why. The table renders Date, Kind, Employer, Position, Channel, Reported, and Notes; the default JSON output carries the `id` the report and exclude verbs take, so read both. `--reported false` narrows a listing to what has not been stamped yet.
