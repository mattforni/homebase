...
| [plumber/](plumber/README.md) | Cloud Run | Monday 22:00 Denver. Pulls the portal, both mailboxes, the One Pager and the candidate sites, runs the `plumber` agent over the pulled files so the week's roster is built and drafted before the Tuesday desk block, and emails a report as `YYYY-Www Pipeline` with the roster attached for Forni to place in the Atelic repo. Rebuilt on the recruiter's shape 2026-09-15 and promoted under the agent's name 2026-09-24 (ATE-551). |
