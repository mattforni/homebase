...
**What the email is.** The agent's whole report, visible in full: the ranked W2 shortlist with one line on what each company does, the considered and rejected list, the fractional lane split into posted roles and prospects, the source notes, and the ledger ready table of every posting judged. The Tuesday block picks from the shortlist, hands the picks to `assist:draft-applications`, and appends the ledger rows to `Craft/Vocation/FY27-sweep-ledger.md`. The cost and turns sit in the footer.
