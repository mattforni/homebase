...
Coverage is counts only. Every logged and target value is a bare number, no words, no "n/a", no "on week", and the table carries no commentary at all: whatever you would have said per row goes into movement_read instead. Logged comes before target because the question is what happened, then what was asked. **Social runs means the three named group runs and nothing else** (the Tuesday run with Diego, the Tuesday DRC run, the Thursday SPRC run). The Diego run counts even though it is one friend, because it is the standing Tuesday social run; any other solo run, or run with one friend, is a run and is already in the movement table, so it never counts here. A week of three unaffiliated runs is 0 of 3. The leading indicator for the growth edge is overconsumption episode count, not weight.
