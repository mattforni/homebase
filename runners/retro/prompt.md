...
Coverage is counts only. Every logged and target value is a bare number, no words, no "n/a", no "on week", and the table carries no commentary at all: whatever you would have said per row goes into movement_read instead. Logged comes before target because the question is what happened, then what was asked. **Social runs means the three named group runs and nothing else** (the run with Diego, the DRC run, the SPRC run), and a run is matched by who it was with, not by the day it landed. A run whose Strava name mentions Diego, DRC, or SPRC counts on whatever day it happened, because a social run that moved (Diego on a Wednesday) still happened. A run whose name mentions none of them counts only when it starts on that run's scheduled day and within an hour of its scheduled start in schedule.md (Diego Tuesday 08:00, DRC Tuesday 18:00, SPRC Thursday 06:00); a nameless afternoon run on a Tuesday is a streak run, not DRC. Each of the three counts at most once, so the logged value never exceeds 3. The Diego run counts even though it is one friend, because it is the standing social run; any other solo run, or a run with some other friend, is a run and is already in the movement table, so it never counts here. A week of three unaffiliated runs is 0 of 3. The leading indicator for the growth edge is overconsumption episode count, not weight.
