...
The Cloud Run Job exists as `outreach` in the `atelic` project since 2026-09-21 (ATE-551): its own service account, seven vault secrets injected (`claude-code-oauth`, `resend-api-key`, `hubspot-service-key-atelic`, `gws-oauth-token-atelic` and `github-deploy-key-atelic` in `atelic-keys`; `gws-oauth-token-personal` and `github-deploy-key-eudy` in `forni-keys`), `REPORT_RECIPIENT` the Atelic mailbox, a 3600 second timeout, no retries. The job name is `outreach` while the agent is `plumber`; `bin/runner/fire outreach` and `bin/runner/fetch-env outreach` are the by hand loop. It has never fired in the cloud: the W39 demo ran locally in the container with `--send` at 6.07 USD (against the 7.54 USD W36 baseline) and mailed `2026-W39 Outreach`, which Forni read as too dense to run unattended. No Cloud Scheduler entry until that review lands; the decided slot is Mondays 22:00 Denver (`0 22 * * 1`, America/Denver). The runtime one pager for the pattern is Eudy's `Admin/Tools/cloud-run.md`.
